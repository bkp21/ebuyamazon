	...
	function ajax_load1(url,id,type){
        var list1 = $('#'+id);
		...
		$.ajax({
			...
			beforeSend: function() {
				//list.fadeOut();
				if(type !== 'other'){
					list1.html(loading); // change submit button text
				}
			},
			success: function(data) {
				if(data !== ''){
					list1.html('');
					list1.html(data).fadeIn(); // fade in response data
				}
				if(type == 'first'){
	...
	function ajax_modal1(type,title,noty,form_id,id){
		modal_form(title,noty,form_id);
		ajax_load1(base_url+'index.php/'+user_type+'/'+module+'/'+type+'/'+id,'form','form');
		sound('ajax_load');
	}
	...
	function ajax_set_list1(extra){
		ajax_load1(base_url+'index.php/'+user_type+'/'+module+'/'+list_cont_funcc+'/'+extra,'list1','first');
		sound('ajax_load1');
	}
	...
	function delete_confirm(id,msg,delete_method){
        if (delete_method !== undefined) {
            dlt_cont_func = delete_method;
        }
        ...
        function delete_confirm1(id,msg,delete_method){
        if (delete_method !== undefined) {
            dlt_cont_funcc = delete_method;
        }

		msg = '<div class="modal-title">'+msg+'</div>';
		bootbox.confirm(msg, function(result) {
			if (result) {
				ajax_load1(base_url+'index.php/'+user_type+'/'+module+'/'+dlt_cont_funcc+'/'+id,'list1','delete1');
				$.activeitNoty({
					type: 'danger',
					icon : 'fa fa-check',
					message : dss,
      ...
      function change_confirm(id,msg){
		msg = '<div class="modal-title">'+msg+'</div>';
		bootbox.confirm(msg, function(result) {
			if (result) {
				ajax_load(base_url+'index.php/'+user_type+'/'+module+'/'+change_cont_func+'/'+id,'list','delete');
                                ajax_load1(base_url+'index.php/'+user_type+'/'+module+'/'+change_cont_funcc+'/'+id,'list1','delete1');
				$.activeitNoty({
					type: 'success',
					icon : 'fa fa-check',
					message : chge,
		...
		bootbox.confirm(msg, function(result) {
			if (result) {
				ajax_load(base_url+'index.php/'+user_type+'/'+module+'/'+func+'/'+id,'list','delete');
                                ajax_load1(base_url+'index.php/'+user_type+'/'+module+'/'+func+'/'+id,'list1','delete1');
				$.activeitNoty({
					type: 'success',
					icon : 'fa fa-check',
					message : noty,
	...
	function export_it1(type,ignore){
		$('#export-table1').show();
		var name = $('#export-table1').data('name');
		var o = $('#export-table1').data('orientation');
		var w = $('#export-table1').data('width');
		if(type == 'pdf'){
			//FromaHTML('export-div',name,o);
			FromHTML('export-div1',name,o,w);
		} else {
			$('#export-table1').tableExport({type:type,escape:'false',tableName:name,pdfFontSize:10,htmlContent:'true',ignoreColumn:'['+ignore+']'});
			$('#export-table1').hide();
		}
	}
