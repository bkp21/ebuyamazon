...
class Crud_model extends CI_Model
{
    ...
    //check maintenance
    function check_maint($setting_id){
    ...
    public function getAllNews( $limit = null, $start = null ){

        $this->db->from( 'vendor_news' );
    ...
    public function countAllNews(){
       // $this->db->where('status',1);
        return $this->db->count_all( 'vendor_news' );
    }
    ...
    public function getAllReview( $limit = null, $start = null ){
    ...
    public function countAllReview(){
        // $this->db->where('status',1);
        return $this->db->count_all( 'vendor_reviews' );
    }

    public function getAllRating( $limit = null, $start = null ){
    ...
    public function countAllRating(){
        // $this->db->where('status',1);
        return $this->db->count_all( 'vendor_ratings' );
    }


    public function getVendorReviewSetting($id = null){
        $this->db->from('vendor_review_setting');
    ...
    public function getVendorRatingSetting($id = null){
        $this->db->from('vendor_rating_setting');
        $this->db->where('setting_id',$id);
        $this->db->where('status',1);
        $q = $this->db->get();
        return $q->row();
    ...
    public function getVendorServiceSetting($id = null){
    ...
    public function getVendorName($id){
        $this->db->select(array('display_name'));
        $this->db->from('vendor');
        $this->db->where('vendor_id',$id);
        $q = $this->db->get();
      //  echo $this->db->last_query(); exit();
        return $q->row()->display_name;

    }

    public function getVendorListForVendorFeeUpdate(){
        $this->db->select(array('vendor_id','display_name'));
        $this->db->from('vendor');
        $q = $this->db->get();
        return $q->result_array();
    }
    ...
    public function getAllVendorServices($limit=null, $start = null){
        $this->db->from("ordered_vendor_services");
    ...
    public function countAllVendorServices(){
        $this->db->count_all( 'ordered_vendor_services' );
    }

    public function getAllVendorFee($date, $limit=null, $start = null ){

        $date = explode("-", $date);
        $this->db->from("vendor_monthly_fees");
        $this->db->where("( YEAR(`start_date`) = {$date[0]} AND MONTH(`start_date`) = {$date[1]} )");
    ...
    public function countAllVendorFee($date){
        $date = explode("-", $date);
        $this->db->where("( YEAR(`start_date`) = {$date[0]} AND MONTH(`start_date`) = {$date[1]} )");
        return $this->db->count_all( 'vendor_monthly_fees' );
    }

    public function vendorFeeInfo($id){
        $this->db->from("vendor_monthly_fees");
    ...
    public function getAllUser( $limit = null, $start = null ){
    ...
    {   
		...
		public function GetUserAccountData($userid)
		{
			$data=array();
   ...
   {   
	   ...
	   $data['account']=$account;
	    ...
	    /*user_billing_info_tbl*/
	   $this->db->where('user_id', $userid);
       $query_billing = $this->db->get('user_billing_info_tbl');
       $billing = $query_billing->result_array();
	   $data['billing']=$billing;

	   /*user_shipping_info_tbl*/
	   $this->db->where('user_id', $userid);
       $query_shipping = $this->db->get('user_shipping_info_tbl');
       $shipping = $query_shipping->result_array();
	   $data['shipping']=$shipping;

	   	   /*user_total_point_tbl*/
	   $this->db->where('user_id', $userid);
       $query_point = $this->db->get('user_total_point_tbl');
       $point = $query_point->result_array();
	   $data['point']=$point;
   ...
   public function search_user($username,$surname,$email,$phone){
    ...
    //single  not empty with username
...
//	  if(!empty($phone) && !empty($acc_company_name) ){$this->db->where('phone', $phone);$this->db->where('acc_company_name', $acc_company_name;}
   ...
   return $query->result_array();
    ...
    public function search_product($productname,$vendor,$category,$approved,$pending,$declined,$stock_warning,$is_template,$tag,$sort){
        //single  not empty with category
  ...
  if (!is_array($user_tree_array))
    ...
    $sql = $this->db->get('category');
     foreach($sql->result() as $row) {
      $user_tree_array[] = array("category_id" => $row->category_id, "category_name" => $spacing . $row->category_name,"parentid" => $row->parentid,);
      $user_tree_array =  $this->crud_model->fetchCategoryTree($row->category_id, $spacing . '&nbsp;&nbsp;&nbsp;', $user_tree_array);
    }
...
function fetchCategoryTreeList($parent = 0, $user_tree_array = '') {
    ...
    $this->db->order_by('parentid','ASC');    
    $sql = $this->db->get('category');
    $user_tree_array[] = "<ul  id='mainnav-menu' class='list-group'>";
     foreach($sql->result() as $row) {
        $user_tree_array[] = "<li class='active-sub active'>". $row->category_name."</li>"; 
        $user_tree_array = $this->crud_model->fetchCategoryTreeList($row->category_id, $user_tree_array);
     }
     $user_tree_array[] = "</ul>";
...
function isLastCategory($category_id){
...
function createTreeView($array, $currentParent, $currLevel = 0, $prevLevel = -1) {
	...
	echo '<li>';
     ...
     $view_i = "";
            if($this->crud_model->isLastCategory($category['category_id']) != 'last'){
                    echo '<input type="checkbox"  value="'.$i.'" id="subfolder_'.$i.'"/>';
                    $view_i = "<i></i>";
            }
	
        echo '<label class="col-lg-8" id="lab"  for="subfolder_'.$i.'"><b>'.$category['category_name'].'</b>'.$view_i.'</label><span style="float:right;"><a class="btn btn-success btn-xs btn-labeled fa fa-wrench" data-toggle="tooltip" onclick="ajax_set_full(\'edit\',\''.translate('edit_category').'\',\''.translate('successfully_edited!').'\',\'category_edit\',\''.$category['category_id'].'\');proceed(\'to_list\');" data-original-title="Edit" data-container="body">'.translate('').'</a><a onclick="delete_confirm(\''.$category['category_id'].'\', \''.translate('really_want_to_delete_this?').'\')" class="btn btn-danger btn-xs btn-labeled fa fa-trash" data-toggle="tooltip" data-original-title="Delete" data-container="body">'.translate('').'</a></span>';
        if ($currLevel > $prevLevel) { $prevLevel = $currLevel; }
        $currLevel++; 

        $this->crud_model->createTreeView($array, $categoryId, $currLevel, $prevLevel);
